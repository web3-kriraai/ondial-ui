...
type MarketingDottedPageShellProps = {
  children: ReactNode;
  className?: string;
};

/** Marketing routes: soft off-white surface + dotted grid + ambient brand glow. */
export function MarketingDottedPageShell({ children, className }: MarketingDottedPageShellProps) {
  return (
    <div className={cn("relative flex min-h-full w-full flex-col", className)}>
      ...
      >
        ...
        <MarketingDotBackground className="inset-0 z-1" />

        <div
          className="absolute inset-x-0 top-0 z-2 h-[min(520px,55vh)] opacity-70"
          style={{
            background:
